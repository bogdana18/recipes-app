...
import { getFavouriteRecipes, saveFavouriteRecipes } from '../helpers/storage';
...
export default function RecipeDetailScreen(props) {
  ...
  const checkIfFavourite = async (idMeal) => {
    ...
    setIsFavourite(favs.some(fav => fav.idMeal === idMeal));
  };
  ...
  const toggleFavourite = async () => {
    ...
    let newFavs;
    if (isFavourite) {
      newFavs = favs.filter(fav => fav.idMeal !== item.idMeal);
    } else {
      newFavs = [...favs, item];
    }
    await saveFavouriteRecipes(newFavs);
    ...
    >
      ...
      {/*Meal desc */}
      {
        loading ? (
          ...
          (
            <Animated.View entering={FadeInDown.duration(700).springify().damping(12)} style={[tw`px-4 flex justify-between pt-8`]}>
                ...
                <View style={[tw`ml-3`, styles.spaceY2]}>
                  {
                    ingredientsIndexes(meal).map(i => {
                      return (
                        <View key={i} style={[tw`flex-row`,styles.spaceX4]}>
              ...
              <Animated.View entering={FadeInDown.delay(300).duration(700).springify().damping(12)} style={styles.spaceY4}>
                <Text style={[{ fontSize: hp(1.5) }, tw`font-bold flex-1 text-neutral-700`, styles.spaceY3]}>
                  Instructions
                </Text>
